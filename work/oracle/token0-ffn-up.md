# Token 0 FFN Up Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_ffn_up_output*_f32_hex` smoke path for the local target GGUF.
...
```sh
python3 work/oracle/token0_ffn_up_oracle.py \
...
Comparison result: these four FFN up projection words match the current
assembly runtime output for `token0_ffn_up_output0_f32_hex` through
`token0_ffn_up_output3_f32_hex` exactly. The intermediate FFN RMSNorm words

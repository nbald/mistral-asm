#!/usr/bin/env python3
"""External oracle for the token-0 layer-3 post-FFN residual smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-3 FFN down oracle path, then adds the layer-3 post-attention residual to
the layer-3 FFN down output with scalar f32 rounding.
"""
...
from token0_layer3_ffn_down_oracle import (
    LAYER3_FFN_DOWN,
    PUBLIC_OUTPUT_WORDS,
    run_oracle as run_layer3_ffn_down_oracle,
)
...
def run_oracle(path, residual_words=PUBLIC_OUTPUT_WORDS):
    result = run_layer3_ffn_down_oracle(path, residual_words)
    post_attn = result["layer3_post_attn_residuals"]
    down = result["layer3_ffn_down_outputs"]
    ...
    result["layer3_post_ffn_residuals"] = residuals
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-3 post-FFN residual words.")
    parser.add_argument("model", help="path to the target Q8_0 GGUF model")
    parser.add_argument(
        "--residual-words", type=int, default=PUBLIC_OUTPUT_WORDS,
        help="number of layer-3 down/residual words to print")
    ...
    for index, value in enumerate(result["layer3_ffn_norm_words"]):
        print(f"oracle_token0_layer3_ffn_norm{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["layer3_ffn_gate_outputs"][:4]):
        print(f"oracle_token0_layer3_ffn_gate_output{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["layer3_ffn_up_outputs"][:4]):
        print(f"oracle_token0_layer3_ffn_up_output{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["layer3_ffn_swiglu_outputs"][:4]):

#!/usr/bin/env python3
"""External oracle for the token-0 layer-3 FFN down projection smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-3 FFN RMSNorm oracle path, computes the full layer-3 FFN gate/up/SwiGLU
...
LAYER3_FFN_DOWN = "blk.3.ffn_down.weight"
PUBLIC_OUTPUT_WORDS = 4


def load_layer3_ffn_down_outputs(path, expected_epsilon_bits, activation,
    ...
    require(0 <= down_rows <= OUTPUT_WIDTH,
            "requested layer-3 FFN down row count outside output width")
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            layer3_ffn_up = require_tensor(
                tensors, LAYER3_FFN_UP, GGML_TYPE_Q8_0, 2)
            layer3_ffn_down = require_tensor(
                tensors, LAYER3_FFN_DOWN, GGML_TYPE_Q8_0, 2)
            require(layer3_ffn_gate["dims"][0] == OUTPUT_WIDTH,
                    "layer-3 FFN gate input width mismatch")
            require(layer3_ffn_gate["dims"][1] == FFN_SWIGLU_WIDTH,
                    "layer-3 FFN gate output width mismatch")
            require(layer3_ffn_up["dims"][0] == OUTPUT_WIDTH,
                    "layer-3 FFN up input width mismatch")
            require(layer3_ffn_up["dims"][1] == FFN_SWIGLU_WIDTH,
                    "layer-3 FFN up output width mismatch")
            require(layer3_ffn_down["dims"][0] == FFN_SWIGLU_WIDTH,
                    "layer-3 FFN down input width mismatch")
            require(layer3_ffn_down["dims"][1] == OUTPUT_WIDTH,
                    "layer-3 FFN down output width mismatch")
            ...
            up_row_bytes = q8_0_row_bytes(layer3_ffn_up["dims"][0])
            up_start = tensor_pointer(
                buf, tensor_data_offset, layer3_ffn_up,
                up_row_bytes * layer3_ffn_up["dims"][1])
            down_row_bytes = q8_0_row_bytes(layer3_ffn_down["dims"][0])
            down_start = tensor_pointer(
                buf, tensor_data_offset, layer3_ffn_down,
                down_row_bytes * layer3_ffn_down["dims"][1])
            ...
            return {
                ...
                LAYER3_FFN_UP: layer3_ffn_up,
                LAYER3_FFN_DOWN: layer3_ffn_down,
                "layer3_ffn_gate_outputs": gate_outputs.copy(),
                "layer3_ffn_up_outputs": up_outputs.copy(),
                "layer3_ffn_swiglu_outputs": swiglu_outputs.copy(),
                "layer3_ffn_down_outputs": down_outputs.copy(),
            }
    finally:
        os.close(fd)
...
def run_oracle(path, down_rows=PUBLIC_OUTPUT_WORDS):
    ...
    down = load_layer3_ffn_down_outputs(
        path,
        result["epsilon_bits"],
...
def main():
    ...
    for index, value in enumerate(result["layer3_ffn_swiglu_outputs"]
        ...
        print(f"oracle_token0_layer3_ffn_swiglu_output{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["layer3_ffn_down_outputs"]):
        print(f"oracle_token0_layer3_ffn_down_output{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")

#!/usr/bin/env python3
"""External oracle for the token-0 layer-1 attention output projection smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-1 attention RMSNorm oracle path, computes the full layer-1 value
projection, expands the one-token grouped-query context, and dots that context
with the first four rows of blk.1.attn_output.weight using the same scalar f32
...
LAYER1_ATTN_OUTPUT = "blk.1.attn_output.weight"
PUBLIC_OUTPUT_WORDS = 4


def load_layer1_attn_output_outputs(path, expected_epsilon_bits, activation):
...
def run_oracle(path):
    ...
    (
        ...
        context_words,
        outputs,
    ) = load_layer1_attn_output_outputs(
    ...
    result[LAYER1_ATTN_OUTPUT] = layer1_attn_output
    result["layer1_attn_context_words"] = context_words
    result["layer1_attn_output_outputs"] = outputs
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-1 attention output words.")

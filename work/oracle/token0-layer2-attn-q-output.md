# Token 0 Layer-2 Attention Query Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer2_attn_q_output*_f32_hex` smoke path for the local target GGUF.
...
`blk.2.attn_q.weight` using the same scalar Q8_0 f32 accumulation order as
`q8_0_matvec_f32`.
...
```sh
python3 work/oracle/token0_layer2_attn_q_oracle.py \
...
blk.2.attn_q.weight: type 8 dims 3072x4096 offset 691937280
...
oracle_token0_layer2_attn_q_output0_f32_hex: 0x3f29ab97
oracle_token0_layer2_attn_q_output1_f32_hex: 0x3fa60667
oracle_token0_layer2_attn_q_output2_f32_hex: 0x4000572f
oracle_token0_layer2_attn_q_output3_f32_hex: 0x3fb6f799
```

Comparison result: these four layer-2 attention query words match the current
assembly runtime output for `token0_layer2_attn_q_output0_f32_hex` through
`token0_layer2_attn_q_output3_f32_hex` exactly. The prerequisite layer-1

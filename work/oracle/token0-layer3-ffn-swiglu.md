# Token 0 Layer-3 FFN SwiGLU Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer3_ffn_swiglu_output*_f32_hex` smoke path for the local target
GGUF.

This oracle is external verification tooling only. It is not linked by the
runtime build. The script reuses the full layer-3 FFN RMSNorm oracle path plus
the focused layer-3 FFN gate/up projection loaders, then applies
`silu(gate[i]) * up[i]` for the first four activation words using the same
scalar helper semantics as the runtime `swiglu_f32` smoke path.
...
8. Dot the FFN-normalized activation with the first four rows of
   `blk.3.ffn_gate.weight` and `blk.3.ffn_up.weight`.
9. Apply `silu(gate[i]) * up[i]` to the first four gate/up pairs.

Command:

```sh
python3 work/oracle/token0_layer3_ffn_swiglu_oracle.py \
...
Comparison result: these four layer-3 FFN SwiGLU words match the current
assembly runtime output for `token0_layer3_ffn_swiglu_output0_f32_hex` through
`token0_layer3_ffn_swiglu_output3_f32_hex` exactly. The prerequisite layer-3
attention output, post-attention residual, FFN RMSNorm, FFN gate, and FFN up

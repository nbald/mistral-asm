# Token 0 Layer-2 Attention Value Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer2_attn_v_output*_f32_hex` smoke path for the local target GGUF.
...
`blk.2.attn_v.weight` using the same scalar Q8_0 f32 accumulation order as
`q8_0_matvec_f32`.
...
```sh
python3 work/oracle/token0_layer2_attn_v_oracle.py \
...
oracle_token0_layer2_attn_v_output0_f32_hex: 0x3d38e19b
oracle_token0_layer2_attn_v_output1_f32_hex: 0x3ae7765b
oracle_token0_layer2_attn_v_output2_f32_hex: 0xbd4bbba8
oracle_token0_layer2_attn_v_output3_f32_hex: 0xbf48b85f
```

Comparison result: these four layer-2 attention value words match the current
assembly runtime output for `token0_layer2_attn_v_output0_f32_hex` through
`token0_layer2_attn_v_output3_f32_hex` exactly. The prerequisite layer-1

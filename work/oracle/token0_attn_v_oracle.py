#!/usr/bin/env python3
"""External oracle for the token-0 first attention value projection smoke.
...
first four value-projection output words as raw bits.
"""
...
def run_oracle(path):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            require(attn_v["dims"][1] >= 4,
                    "attention value tensor has fewer than four rows")
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 attention value oracle words.")
    ...
    for label in (TOKEN_EMBD, ATTN_NORM, ATTN_V):
        tensor = result[label]
    ...
    for index, value in enumerate(result["outputs"]):
        print(f"oracle_token0_attn_v_output{index}_f32_hex: "

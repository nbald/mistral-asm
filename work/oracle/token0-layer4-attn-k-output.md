# Token 0 Layer-4 Attention Key Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer4_attn_k_output*_f32_hex` smoke path for the local target GGUF.
...
`blk.4.attn_k.weight` using the same scalar f32 Q8_0 accumulation order as
`q8_0_matvec_f32`.
...
```sh
python3 work/oracle/token0_layer4_attn_k_oracle.py \
...
oracle_token0_layer4_attn_k_output0_f32_hex: 0xbc326305
oracle_token0_layer4_attn_k_output1_f32_hex: 0x3c2ff2f1
oracle_token0_layer4_attn_k_output2_f32_hex: 0x3a8970d8
oracle_token0_layer4_attn_k_output3_f32_hex: 0xbc83c191
```

Comparison result: these four layer-4 attention key words match the current
assembly runtime output for `token0_layer4_attn_k_output0_f32_hex` through
`token0_layer4_attn_k_output3_f32_hex` exactly. The prerequisite layer-3

# Token 0 Attention Value Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_attn_v_output*_f32_hex` smoke path for the local target GGUF.
...
```sh
python3 work/oracle/token0_attn_v_oracle.py \
...
oracle_token0_attn_v_output0_f32_hex: 0x3ca3b3bc
oracle_token0_attn_v_output1_f32_hex: 0x3c9bf3e4
oracle_token0_attn_v_output2_f32_hex: 0x3c29a3e4
oracle_token0_attn_v_output3_f32_hex: 0xbb17585e
```

Comparison result: these four words match the current assembly runtime output
for `token0_attn_v_output0_f32_hex` through
`token0_attn_v_output3_f32_hex` exactly.

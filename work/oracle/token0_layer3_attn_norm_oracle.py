#!/usr/bin/env python3
"""External oracle for the token-0 layer-3 attention RMSNorm smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-2 post-FFN residual oracle path to compute the full 3072-word residual,
then applies blk.3.attn_norm.weight with the same RMSNorm helper semantics used
...
from token0_layer2_post_ffn_residual_oracle import (
    run_oracle as run_layer2_post_ffn_residual_oracle,
)


LAYER3_ATTN_NORM = "blk.3.attn_norm.weight"
PUBLIC_OUTPUT_WORDS = 4


def load_layer3_attn_norm(path, expected_epsilon_bits):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            layer3_attn_norm = require_tensor(
                tensors, LAYER3_ATTN_NORM, GGML_TYPE_F32, 1)
            require(layer3_attn_norm["dims"][0] == OUTPUT_WIDTH,
                    "layer-3 attention RMSNorm width mismatch")

            norm_start = tensor_pointer(
                buf, tensor_data_offset, layer3_attn_norm, OUTPUT_WIDTH * 4)
            weights = np.frombuffer(
                buf, dtype="<f4", count=OUTPUT_WIDTH,
                offset=norm_start).copy()

            return tensor_data_offset, layer3_attn_norm, weights
    finally:
        os.close(fd)


def run_oracle(path):
    result = run_layer2_post_ffn_residual_oracle(
        path, residual_words=OUTPUT_WIDTH)
    tensor_data_offset, layer3_attn_norm, weights = load_layer3_attn_norm(
        path, result["epsilon_bits"])

    result["tensor_data_offset"] = tensor_data_offset
    result[LAYER3_ATTN_NORM] = layer3_attn_norm
    result["layer3_attn_norm_activation"] = rmsnorm(
        result["layer2_post_ffn_residuals"], weights,
        f32_from_bits(result["epsilon_bits"]))
    result["layer3_attn_norm_words"] = (
        result["layer3_attn_norm_activation"][:PUBLIC_OUTPUT_WORDS].copy())
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-3 attention RMSNorm words.")
    ...
    for label in (TOKEN_EMBD, ATTN_NORM, ATTN_V, ATTN_OUTPUT, FFN_NORM,
                  ...
                  LAYER2_FFN_DOWN, LAYER3_ATTN_NORM):
        tensor = result[label]

# Token 0 Layer-3 Attention Query Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer3_attn_q_output*_f32_hex` smoke path for the local target GGUF.

This oracle is external verification tooling only. It is not linked by the
runtime build. The script reuses the full layer-3 attention RMSNorm oracle path,
then dots the resulting 3072-word activation with the first four rows of
`blk.3.attn_q.weight` using the same scalar Q8_0 f32 accumulation order as
`q8_0_matvec_f32`.
...
```sh
python3 work/oracle/token0_layer3_attn_q_oracle.py \
...
blk.3.attn_q.weight: type 8 dims 3072x4096 offset 815628288
...
oracle_token0_layer3_attn_q_output0_f32_hex: 0x3de458d2
oracle_token0_layer3_attn_q_output1_f32_hex: 0x3eae6d55
oracle_token0_layer3_attn_q_output2_f32_hex: 0x3d06883d
oracle_token0_layer3_attn_q_output3_f32_hex: 0xbe14568c
```

Comparison result: these four layer-3 attention query words match the current
assembly runtime output for `token0_layer3_attn_q_output0_f32_hex` through
`token0_layer3_attn_q_output3_f32_hex` exactly. The prerequisite layer-2
post-FFN residual and layer-3 attention RMSNorm public slices also still match
exactly.

#!/usr/bin/env python3
"""External oracle for the token-0 layer-5 FFN up projection smoke.
...
the first four rows of blk.5.ffn_up.weight using the same scalar f32 Q8_0
...
from token0_layer5_ffn_gate_oracle import (
    LAYER5_FFN_GATE,
    load_layer5_ffn_gate_outputs,
)
...
LAYER5_FFN_UP = "blk.5.ffn_up.weight"


def load_layer5_ffn_up_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            require(layer5_ffn_up["dims"][1] == FFN_UP_OUTPUT_WIDTH,
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-5 FFN up words.")

#!/usr/bin/env python3
"""External oracle for the token-0 layer-2 FFN gate projection smoke.
...
the first four rows of blk.2.ffn_gate.weight using the same scalar f32 Q8_0
...
LAYER2_FFN_GATE = "blk.2.ffn_gate.weight"
PUBLIC_OUTPUT_WORDS = 4


def load_layer2_ffn_gate_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            require(layer2_ffn_gate["dims"][1] == FFN_GATE_OUTPUT_WIDTH,
            ...
            return tensor_data_offset, layer2_ffn_gate, outputs
    finally:
        os.close(fd)
...
def run_oracle(path):
    ...
    tensor_data_offset, layer2_ffn_gate, outputs = (
    ...
    result["layer2_ffn_gate_outputs"] = outputs
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-2 FFN gate words.")
    ...
    for label in (TOKEN_EMBD, ATTN_NORM, ATTN_V, ATTN_OUTPUT, FFN_NORM,
                  ...
                  LAYER2_FFN_NORM, LAYER2_FFN_GATE):
        tensor = result[label]

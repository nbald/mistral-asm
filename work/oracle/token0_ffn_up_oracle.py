#!/usr/bin/env python3
"""External oracle for the token-0 first FFN up projection smoke.
...
def run_oracle(path):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            up_outputs = []
            for row in range(4):
                row_start = ffn_up_start + row * ffn_up_row_bytes
                up_outputs.append(q8_0_dot_row(buf, row_start,
                                               ffn_norm_output))
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 FFN up oracle words.")

#!/usr/bin/env python3
"""External oracle for the token-0 layer-4 FFN up projection smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-4 FFN RMSNorm oracle path, then dots that full 3072-word activation with
the first four rows of blk.4.ffn_up.weight using the same scalar f32 Q8_0
...
from token0_ffn_up_oracle import FFN_UP_OUTPUT_WIDTH
...
from token0_layer4_ffn_gate_oracle import (
    LAYER4_FFN_GATE,
    load_layer4_ffn_gate_outputs,
)
...
LAYER4_FFN_UP = "blk.4.ffn_up.weight"
PUBLIC_OUTPUT_WORDS = 4


def load_layer4_ffn_up_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            require(layer4_ffn_up["dims"][1] == FFN_UP_OUTPUT_WIDTH,
...
def run_oracle(path):
    ...
    gate = load_layer4_ffn_gate_outputs(
        path,
        result["epsilon_bits"],
        result["layer4_ffn_norm_activation"],
    )
    up = load_layer4_ffn_up_outputs(
        path,
        result["epsilon_bits"],
        result["layer4_ffn_norm_activation"],
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-4 FFN up words.")

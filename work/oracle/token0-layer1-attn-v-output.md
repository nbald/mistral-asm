# Token 0 Layer-1 Attention Value Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer1_attn_v_output*_f32_hex` smoke path for the local target GGUF.
...
`blk.1.attn_v.weight` using the same scalar Q8_0 f32 accumulation order as
`q8_0_matvec_f32`.
...
Comparison result: these four layer-1 attention value words match the current
assembly runtime output for `token0_layer1_attn_v_output0_f32_hex` through
`token0_layer1_attn_v_output3_f32_hex` exactly. The prerequisite post-FFN

#!/usr/bin/env python3
"""External oracle for the token-0 layer-3 attention query projection smoke.
...
then dots that activation with the first four rows of blk.3.attn_q.weight using
...
LAYER3_ATTN_Q = "blk.3.attn_q.weight"
LAYER3_ATTN_Q_OUTPUT_WIDTH = 4096
PUBLIC_OUTPUT_WORDS = 4


def load_layer3_attn_q_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            layer3_attn_q = require_tensor(
                tensors, LAYER3_ATTN_Q, GGML_TYPE_Q8_0, 2)
            require(layer3_attn_q["dims"][0] == OUTPUT_WIDTH,
                    "layer-3 attention query input width mismatch")
            require(layer3_attn_q["dims"][1] == LAYER3_ATTN_Q_OUTPUT_WIDTH,
                    "layer-3 attention query output width mismatch")

            row_bytes = q8_0_row_bytes(layer3_attn_q["dims"][0])
            matrix_start = tensor_pointer(
                buf, tensor_data_offset, layer3_attn_q,
                row_bytes * layer3_attn_q["dims"][1])
            ...
            return tensor_data_offset, layer3_attn_q, outputs
    finally:
        os.close(fd)
...
def run_oracle(path):
    ...
    tensor_data_offset, layer3_attn_q, outputs = load_layer3_attn_q_outputs(
        path, result["epsilon_bits"], result["layer3_attn_norm_activation"])

    result["tensor_data_offset"] = tensor_data_offset
    result[LAYER3_ATTN_Q] = layer3_attn_q
    result["layer3_attn_q_outputs"] = outputs
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-3 attention query words.")
    ...
    for label in (TOKEN_EMBD, ATTN_NORM, ATTN_V, ATTN_OUTPUT, FFN_NORM,
                  ...
                  LAYER2_FFN_DOWN, LAYER3_ATTN_NORM, LAYER3_ATTN_Q):
        tensor = result[label]
    ...
    for index, value in enumerate(result["layer3_attn_norm_words"]):
        print(f"oracle_token0_layer3_attn_norm{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["layer3_attn_q_outputs"]):
        print(f"oracle_token0_layer3_attn_q_output{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")

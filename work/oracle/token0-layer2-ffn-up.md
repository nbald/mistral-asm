# Token 0 Layer-2 FFN Up Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer2_ffn_up_output*_f32_hex` smoke path for the local target GGUF.
...
`blk.2.ffn_up.weight` using the same scalar f32 Q8_0 accumulation order as the
runtime `q8_0_matvec_f32` helper.
...
```sh
python3 work/oracle/token0_layer2_ffn_up_oracle.py \
...
Comparison result: these four layer-2 FFN up words match the current assembly
runtime output for `token0_layer2_ffn_up_output0_f32_hex` through
`token0_layer2_ffn_up_output3_f32_hex` exactly. The prerequisite layer-2 FFN

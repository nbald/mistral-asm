#!/usr/bin/env python3
"""External oracle for the token-0 layer-2 attention value projection smoke.
...
then dots that activation with the first four rows of blk.2.attn_v.weight using
...
LAYER2_ATTN_V = "blk.2.attn_v.weight"
LAYER2_ATTN_V_OUTPUT_WIDTH = 1024
PUBLIC_OUTPUT_WORDS = 4


def load_layer2_attn_v_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            require(layer2_attn_v["dims"][1] == LAYER2_ATTN_V_OUTPUT_WIDTH,
                    "layer-2 attention value output width mismatch")

            row_bytes = q8_0_row_bytes(layer2_attn_v["dims"][0])
            matrix_start = tensor_pointer(
                buf, tensor_data_offset, layer2_attn_v,
                row_bytes * layer2_attn_v["dims"][1])
            ...
            return tensor_data_offset, layer2_attn_v, outputs
    finally:
        os.close(fd)
...
def run_oracle(path):
    ...
    tensor_data_offset, layer2_attn_v, outputs = load_layer2_attn_v_outputs(
    ...
    result["layer2_attn_v_outputs"] = outputs
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-2 attention value words.")
    ...
    for label in (TOKEN_EMBD, ATTN_NORM, ATTN_V, ATTN_OUTPUT, FFN_NORM,
                  ...
                  LAYER2_ATTN_NORM, LAYER2_ATTN_V):
        tensor = result[label]
    ...
    for index, value in enumerate(result["layer2_attn_norm_words"]):
        print(f"oracle_token0_layer2_attn_norm{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["layer2_attn_v_outputs"]):
        print(f"oracle_token0_layer2_attn_v_output{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")

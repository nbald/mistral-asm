#!/usr/bin/env python3
"""External oracle for the token-0 first attention output projection smoke.

This file is verification tooling, not runtime source. It reuses the GGUF
parser and scalar f32 arithmetic helpers from the query oracle, derives the
single-token attention context from the value projection, and prints the first
four output-projection words as raw bits.
"""
...
ATTN_V = "blk.0.attn_v.weight"
ATTN_OUTPUT = "blk.0.attn_output.weight"

KV_HEADS = 8
QUERY_HEADS_PER_KV_HEAD = 4
HEAD_DIM = 128
VALUE_OUTPUT_WIDTH = KV_HEADS * HEAD_DIM
CONTEXT_WIDTH = KV_HEADS * QUERY_HEADS_PER_KV_HEAD * HEAD_DIM
OUTPUT_WIDTH = 3072


def expand_single_token_context(value_output):
    require(len(value_output) == VALUE_OUTPUT_WIDTH,
            "value output width mismatch")

    context = np.empty(CONTEXT_WIDTH, dtype=np.float32)
    out_index = 0
    for kv_head in range(KV_HEADS):
        head_start = kv_head * HEAD_DIM
        head_end = head_start + HEAD_DIM
        for _ in range(QUERY_HEADS_PER_KV_HEAD):
            context[out_index:out_index + HEAD_DIM] = value_output[
                head_start:head_end]
            out_index += HEAD_DIM
...
def run_oracle(path):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            normalized = rmsnorm(token0, norm_weight,
            ...
            for row in range(4):
                ...
                outputs.append(q8_0_dot_row(buf, row_start, context))
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 attention output oracle words.")
    ...
    for label in (TOKEN_EMBD, ATTN_NORM, ATTN_V, ATTN_OUTPUT):
        tensor = result[label]
        dims = "x".join(str(dim) for dim in tensor["dims"])
        print(f"{label}: type {tensor['type']} dims {dims} "
              f"offset {tensor['offset']}")
    for index, value in enumerate(result["context_words"]):
        print(f"oracle_token0_attn_context{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["outputs"]):
        print(f"oracle_token0_attn_output{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")

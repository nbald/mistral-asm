# Token 0 Layer-2 Post-FFN Residual Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer2_post_ffn_residual*_f32_hex` smoke path for the local target
GGUF.

This oracle is external verification tooling only. It is not linked by the
runtime build. The script reuses the full layer-2 FFN down oracle path, then
adds the first four layer-2 post-attention residual words to the first four
layer-2 FFN down output words with f32 rounding.

The arithmetic chain checked here extends
`work/oracle/token0-layer2-ffn-down.md`:

1. Compute the complete token-0 layer-2 post-attention residual.
2. Compute the complete token-0 layer-2 FFN RMSNorm activation.
3. Dot that activation with every row of `blk.2.ffn_gate.weight`.
4. Dot that activation with every row of `blk.2.ffn_up.weight`.
5. Compute the complete layer-2 FFN SwiGLU activation with the same scalar
   helper used by the earlier FFN oracles.
6. Dot the SwiGLU activation with the first four rows of
   `blk.2.ffn_down.weight`.
7. Add the first four layer-2 post-attention residual and FFN down words with
   f32 rounding.

Command:

```sh
python3 work/oracle/token0_layer2_post_ffn_residual_oracle.py \
...
Comparison result: these four layer-2 post-FFN residual words match the
current assembly runtime output for `token0_layer2_post_ffn_residual0_f32_hex`
through `token0_layer2_post_ffn_residual3_f32_hex` exactly. The prerequisite
layer-2 post-attention residual and layer-2 FFN public slices also still match
exactly.

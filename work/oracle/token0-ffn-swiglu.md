# Token 0 FFN SwiGLU Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_ffn_swiglu_output*_f32_hex` smoke path for the local target GGUF.
...
8. Dot the resulting FFN-normalized activation with the first four rows of
   both `blk.0.ffn_gate.weight` and `blk.0.ffn_up.weight`.
9. Require the `blk.0.ffn_down.weight` descriptor shape that guards the
   assembly SwiGLU smoke, then compute `silu(gate[i]) * up[i]` for the first
   four FFN activation words.

Command:

```sh
python3 work/oracle/token0_ffn_swiglu_oracle.py \
...
Comparison result: these four FFN SwiGLU activation words match the current
assembly runtime output for `token0_ffn_swiglu_output0_f32_hex` through
`token0_ffn_swiglu_output3_f32_hex` exactly. The intermediate FFN RMSNorm,
gate projection, and up projection words also match the runtime's guarded
public slices exactly.

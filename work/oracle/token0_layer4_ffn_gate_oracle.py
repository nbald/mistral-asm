#!/usr/bin/env python3
"""External oracle for the token-0 layer-4 FFN gate projection smoke.
...
the first four rows of blk.4.ffn_gate.weight using the same scalar f32 Q8_0
...
LAYER4_FFN_GATE = "blk.4.ffn_gate.weight"
PUBLIC_OUTPUT_WORDS = 4


def load_layer4_ffn_gate_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            require(layer4_ffn_gate["dims"][1] == FFN_GATE_OUTPUT_WIDTH,
            ...
            return {
                ...
                "layer4_ffn_gate_outputs": outputs,
            }
    finally:
        os.close(fd)
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-4 FFN gate words.")

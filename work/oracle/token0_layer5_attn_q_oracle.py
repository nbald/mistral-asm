#!/usr/bin/env python3
"""External oracle for the token-0 layer-5 attention query projection smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-5 attention RMSNorm oracle path, then dots that full 3072-word activation
with the first four rows of blk.5.attn_q.weight using the same scalar f32 Q8_0
...
from token0_layer4_ffn_down_oracle import LAYER4_FFN_DOWN
...
from token0_layer5_attn_norm_oracle import (
    LAYER5_ATTN_NORM,
    run_oracle as run_layer5_attn_norm_oracle,
)


LAYER5_ATTN_Q = "blk.5.attn_q.weight"
LAYER5_ATTN_Q_OUTPUT_WIDTH = 4096
PUBLIC_OUTPUT_WORDS = 4


def load_layer5_attn_q_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            layer5_attn_q = require_tensor(
                tensors, LAYER5_ATTN_Q, GGML_TYPE_Q8_0, 2)
            require(layer5_attn_q["dims"][0] == OUTPUT_WIDTH,
                    "layer-5 attention query input width mismatch")
            require(layer5_attn_q["dims"][1] == LAYER5_ATTN_Q_OUTPUT_WIDTH,
                    "layer-5 attention query output width mismatch")

            row_bytes = q8_0_row_bytes(layer5_attn_q["dims"][0])
            matrix_start = tensor_pointer(
                buf, tensor_data_offset, layer5_attn_q,
                row_bytes * layer5_attn_q["dims"][1])
            ...
            return {
                ...
                LAYER5_ATTN_Q: layer5_attn_q,
                "layer5_attn_q_outputs": outputs,
            }
    finally:
        os.close(fd)
...
def run_oracle(path):
    result = run_layer5_attn_norm_oracle(path)
    query = load_layer5_attn_q_outputs(
        path,
        result["epsilon_bits"],
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-5 attention query words.")

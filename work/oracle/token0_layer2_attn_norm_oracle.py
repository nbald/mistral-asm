#!/usr/bin/env python3
"""External oracle for the token-0 layer-2 attention RMSNorm smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-1 post-FFN residual oracle path to compute the full 3072-word residual
after layer 1, then applies blk.2.attn_norm.weight with the same RMSNorm helper
...
from token0_layer1_post_ffn_residual_oracle import (
    ...
    LAYER1_FFN_GATE,
    LAYER1_FFN_UP,
    run_oracle as run_layer1_post_ffn_residual_oracle,
)


LAYER2_ATTN_NORM = "blk.2.attn_norm.weight"


def load_layer2_attn_norm(path, expected_epsilon_bits):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            layer2_attn_norm = require_tensor(
                tensors, LAYER2_ATTN_NORM, GGML_TYPE_F32, 1)
            require(layer2_attn_norm["dims"][0] == OUTPUT_WIDTH,
                    "layer-2 attention RMSNorm width mismatch")

            norm_start = tensor_pointer(
                buf, tensor_data_offset, layer2_attn_norm, OUTPUT_WIDTH * 4)
            weights = np.frombuffer(
                buf, dtype="<f4", count=OUTPUT_WIDTH,
                offset=norm_start).copy()

            return tensor_data_offset, layer2_attn_norm, weights
    finally:
        os.close(fd)


def run_oracle(path):
    result = run_layer1_post_ffn_residual_oracle(
        path, residual_words=OUTPUT_WIDTH)
    tensor_data_offset, layer2_attn_norm, weights = load_layer2_attn_norm(
        path, result["epsilon_bits"])

    result["tensor_data_offset"] = tensor_data_offset
    result[LAYER2_ATTN_NORM] = layer2_attn_norm
    result["layer2_attn_norm_activation"] = rmsnorm(
        result["layer1_post_ffn_residuals"], weights,
        f32_from_bits(result["epsilon_bits"]))
    result["layer2_attn_norm_words"] = (
        result["layer2_attn_norm_activation"][:4].copy())
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-2 attention RMSNorm words.")

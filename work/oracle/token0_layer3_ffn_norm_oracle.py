#!/usr/bin/env python3
"""External oracle for the token-0 layer-3 FFN RMSNorm smoke.

This file is verification tooling, not runtime source. It recomputes the full
layer-3 attention output and post-attention residual, then applies
blk.3.ffn_norm.weight with the same RMSNorm helper semantics used by the
assembly smoke.
"""
...
LAYER3_FFN_NORM = "blk.3.ffn_norm.weight"
PUBLIC_OUTPUT_WORDS = 4


def load_layer3_ffn_norm(path, expected_epsilon_bits, attn_activation,
                         layer2_post_ffn_residuals):
    require(len(attn_activation) == OUTPUT_WIDTH,
            "layer-3 attention RMSNorm activation width mismatch")
    require(len(layer2_post_ffn_residuals) == OUTPUT_WIDTH,
            "layer-2 post-FFN residual width mismatch")
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            layer3_ffn_norm = require_tensor(
                tensors, LAYER3_FFN_NORM, GGML_TYPE_F32, 1)
            ...
            require(layer3_ffn_norm["dims"][0] == OUTPUT_WIDTH,
                    "layer-3 FFN RMSNorm width mismatch")
            ...
            ffn_norm_start = tensor_pointer(
                buf, tensor_data_offset, layer3_ffn_norm, OUTPUT_WIDTH * 4)
            ...
            for index in range(OUTPUT_WIDTH):
                post_attn_residual[index] = f32(
                    layer2_post_ffn_residuals[index] + attn_output[index])
            ...
            return {
                ...
                LAYER3_FFN_NORM: layer3_ffn_norm,
                "layer3_attn_output_outputs": attn_output.copy(),
                "layer3_post_attn_residuals": post_attn_residual.copy(),
                "layer3_ffn_norm_activation": ffn_norm_output.copy(),
                "layer3_ffn_norm_words": ffn_norm_output[
...
def run_oracle(path):
    ...
    ffn_norm = load_layer3_ffn_norm(
        path,
        result["epsilon_bits"],
        result["layer3_attn_norm_activation"],
        result["layer2_post_ffn_residuals"],
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-3 FFN RMSNorm words.")
    ...
    for label in (TOKEN_EMBD, ATTN_NORM, ATTN_V, ATTN_OUTPUT, FFN_NORM,
                  ...
                  LAYER3_ATTN_OUTPUT, LAYER3_FFN_NORM):
        tensor = result[label]

#!/usr/bin/env python3
"""External oracle for the token-0 layer-4 attention key projection smoke.
...
with the first four rows of blk.4.attn_k.weight using the same scalar f32 Q8_0
...
LAYER4_ATTN_K = "blk.4.attn_k.weight"
LAYER4_ATTN_K_OUTPUT_WIDTH = 1024
PUBLIC_OUTPUT_WORDS = 4


def load_layer4_attn_k_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            row_bytes = q8_0_row_bytes(layer4_attn_k["dims"][0])
            matrix_start = tensor_pointer(
                buf, tensor_data_offset, layer4_attn_k,
                row_bytes * layer4_attn_k["dims"][1])
            ...
            return tensor_data_offset, layer4_attn_k, outputs
    finally:
        os.close(fd)
...
def run_oracle(path):
    ...
    tensor_data_offset, layer4_attn_k, outputs = load_layer4_attn_k_outputs(
        path, result["epsilon_bits"], result["layer4_attn_norm_activation"])

    result["tensor_data_offset"] = tensor_data_offset
    result[LAYER4_ATTN_K] = layer4_attn_k
    result["layer4_attn_k_outputs"] = outputs
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-4 attention key words.")
    ...
    for index, value in enumerate(result["layer4_attn_norm_words"]):
        print(f"oracle_token0_layer4_attn_norm{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["layer4_attn_k_outputs"]):
        print(f"oracle_token0_layer4_attn_k_output{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")

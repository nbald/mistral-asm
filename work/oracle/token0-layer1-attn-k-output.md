# Token 0 Layer-1 Attention Key Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer1_attn_k_output*_f32_hex` smoke path for the local target GGUF.
...
`blk.1.attn_k.weight` using the same scalar Q8_0 f32 accumulation order as
`q8_0_matvec_f32`.
...
```sh
python3 work/oracle/token0_layer1_attn_k_oracle.py \
...
blk.1.attn_k.weight: type 8 dims 3072x1024 offset 551522304
...
oracle_token0_layer1_attn_k_output0_f32_hex: 0x3fb2a129
oracle_token0_layer1_attn_k_output1_f32_hex: 0x405dbdbe
oracle_token0_layer1_attn_k_output2_f32_hex: 0x3f5611d3
oracle_token0_layer1_attn_k_output3_f32_hex: 0x3f1e325d
```

Comparison result: these four layer-1 attention key words match the current
assembly runtime output for `token0_layer1_attn_k_output0_f32_hex` through
`token0_layer1_attn_k_output3_f32_hex` exactly. The prerequisite post-FFN

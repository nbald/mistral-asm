# Token 0 Layer-2 FFN SwiGLU Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer2_ffn_swiglu_output*_f32_hex` smoke path for the local target
GGUF.
...
dots the resulting 3072-word activation with the first four rows of both
`blk.2.ffn_gate.weight` and `blk.2.ffn_up.weight` using the same scalar f32
Q8_0 accumulation order as the runtime `q8_0_matvec_f32` helper. It then
applies the same scalar SwiGLU formula as `swiglu_f32`:
`silu(gate[i]) * up[i]`.
...
8. Dot the FFN-normalized activation with the first four rows of
   `blk.2.ffn_gate.weight` and `blk.2.ffn_up.weight`.
9. Compute `silu(gate[i]) * up[i]` for the first four FFN activation words.

Command:

```sh
python3 work/oracle/token0_layer2_ffn_swiglu_oracle.py \
...
Comparison result: these four layer-2 FFN SwiGLU activation words match the
current assembly runtime output for `token0_layer2_ffn_swiglu_output0_f32_hex`
through `token0_layer2_ffn_swiglu_output3_f32_hex` exactly. The prerequisite
layer-2 FFN RMSNorm, gate projection, and up projection public slices also
still match exactly.

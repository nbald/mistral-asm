# Token 0 Layer-3 Attention Key Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_layer3_attn_k_output*_f32_hex` smoke path for the local target GGUF.
...
`blk.3.attn_k.weight` using the same scalar Q8_0 f32 accumulation order as
`q8_0_matvec_f32`.
...
```sh
python3 work/oracle/token0_layer3_attn_k_oracle.py \
...
blk.3.attn_k.weight: type 8 dims 3072x1024 offset 798904320
...
oracle_token0_layer3_attn_k_output0_f32_hex: 0xbaf936b2
oracle_token0_layer3_attn_k_output1_f32_hex: 0xbcf1bab9
oracle_token0_layer3_attn_k_output2_f32_hex: 0x3c7af998
oracle_token0_layer3_attn_k_output3_f32_hex: 0x3c825ee2
```

Comparison result: these four layer-3 attention key words match the current
assembly runtime output for `token0_layer3_attn_k_output0_f32_hex` through
`token0_layer3_attn_k_output3_f32_hex` exactly. The prerequisite layer-2

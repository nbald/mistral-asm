# Token 0 Attention Key Oracle

Purpose: independently check the first four f32 words printed by the assembly
`token0_attn_k_output*_f32_hex` smoke path for the local target GGUF.
...
```sh
python3 work/oracle/token0_attn_k_oracle.py \
...
blk.0.attn_k.weight: type 8 dims 3072x1024 offset 427831296
oracle_token0_attn_k_output0_f32_hex: 0xc028a3e3
oracle_token0_attn_k_output1_f32_hex: 0x3daaeb62
oracle_token0_attn_k_output2_f32_hex: 0xbe8a8c69
oracle_token0_attn_k_output3_f32_hex: 0xc01d0994
```

Comparison result: these four words match the current assembly runtime output
for `token0_attn_k_output0_f32_hex` through
`token0_attn_k_output3_f32_hex` exactly.

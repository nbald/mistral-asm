#!/usr/bin/env python3
"""External oracle for the token-0 layer-3 FFN up projection smoke.
...
the first four rows of blk.3.ffn_up.weight using the same scalar f32 Q8_0
...
LAYER3_FFN_UP = "blk.3.ffn_up.weight"
FFN_UP_OUTPUT_WIDTH = 9216
PUBLIC_OUTPUT_WORDS = 4


def load_layer3_ffn_up_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            require(layer3_ffn_up["dims"][1] == FFN_UP_OUTPUT_WIDTH,
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-3 FFN up words.")
    ...
    for label in (TOKEN_EMBD, ATTN_NORM, ATTN_V, ATTN_OUTPUT, FFN_NORM,
                  ...
                  LAYER3_ATTN_OUTPUT, LAYER3_FFN_NORM, LAYER3_FFN_UP):
        tensor = result[label]

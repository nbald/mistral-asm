#!/usr/bin/env python3
"""External oracle for the token-0 layer-5 attention value projection smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-5 attention key oracle path to preserve all earlier public slice coverage,
then dots the same full 3072-word RMSNorm activation with the first four rows
of blk.5.attn_v.weight using the same scalar f32 Q8_0 accumulation order as
...
from token0_layer5_attn_k_oracle import (
    ATTN_NORM,
    ATTN_OUTPUT,
    ATTN_V,
    FFN_DOWN,
    ...
    PUBLIC_OUTPUT_WORDS,
    TOKEN_EMBD,
    run_oracle as run_layer5_attn_k_oracle,
)


LAYER5_ATTN_V = "blk.5.attn_v.weight"
LAYER5_ATTN_V_OUTPUT_WIDTH = 1024


def load_layer5_attn_v_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            row_bytes = q8_0_row_bytes(layer5_attn_v["dims"][0])
            matrix_start = tensor_pointer(
                buf, tensor_data_offset, layer5_attn_v,
                row_bytes * layer5_attn_v["dims"][1])
...
def run_oracle(path):
    result = run_layer5_attn_k_oracle(path)
    value = load_layer5_attn_v_outputs(
        path,
        result["epsilon_bits"],
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-5 attention value words.")

#!/usr/bin/env python3
"""External oracle for the token-0 layer-4 post-attention residual smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-4 attention output oracle path, then adds the layer-3 post-FFN residual
to the layer-4 attention output with scalar f32 rounding for the public words.
"""
...
from token0_layer4_attn_output_oracle import (
    LAYER4_ATTN_OUTPUT,
    PUBLIC_OUTPUT_WORDS,
    run_oracle as run_layer4_attn_output_oracle,
)
...
def run_oracle(path):
    result = run_layer4_attn_output_oracle(path)

    residuals = np.empty(PUBLIC_OUTPUT_WORDS, dtype=np.float32)
    for index in range(PUBLIC_OUTPUT_WORDS):
        residuals[index] = f32(
            result["layer3_post_ffn_residuals"][index] +
            result["layer4_attn_output_outputs"][index])

    result["layer4_post_attn_residuals"] = residuals
    return result


def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-4 post-attention residuals.")

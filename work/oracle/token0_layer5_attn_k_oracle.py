#!/usr/bin/env python3
"""External oracle for the token-0 layer-5 attention key projection smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-5 attention query oracle path to preserve all earlier public slice
coverage, then dots the same full 3072-word RMSNorm activation with the first
four rows of blk.5.attn_k.weight using the same scalar f32 Q8_0 accumulation
...
from token0_layer5_attn_q_oracle import (
    ATTN_NORM,
    ATTN_OUTPUT,
    ATTN_V,
    FFN_DOWN,
    ...
    PUBLIC_OUTPUT_WORDS,
    TOKEN_EMBD,
    run_oracle as run_layer5_attn_q_oracle,
)


LAYER5_ATTN_K = "blk.5.attn_k.weight"
LAYER5_ATTN_K_OUTPUT_WIDTH = 1024


def load_layer5_attn_k_outputs(path, expected_epsilon_bits, activation):
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            layer5_attn_k = require_tensor(
                tensors, LAYER5_ATTN_K, GGML_TYPE_Q8_0, 2)
            require(layer5_attn_k["dims"][0] == OUTPUT_WIDTH,
                    "layer-5 attention key input width mismatch")
            require(layer5_attn_k["dims"][1] == LAYER5_ATTN_K_OUTPUT_WIDTH,
                    "layer-5 attention key output width mismatch")

            row_bytes = q8_0_row_bytes(layer5_attn_k["dims"][0])
            matrix_start = tensor_pointer(
                buf, tensor_data_offset, layer5_attn_k,
                row_bytes * layer5_attn_k["dims"][1])
            ...
            return {
                ...
                LAYER5_ATTN_K: layer5_attn_k,
                "layer5_attn_k_outputs": outputs,
            }
    finally:
        os.close(fd)
...
def run_oracle(path):
    result = run_layer5_attn_q_oracle(path)
    key = load_layer5_attn_k_outputs(
        path,
        result["epsilon_bits"],
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 layer-5 attention key words.")

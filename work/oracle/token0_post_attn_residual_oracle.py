#!/usr/bin/env python3
"""External oracle for the token-0 post-attention residual smoke.
...
attention-output smoke path, then adds the token embedding activation to the
first four attention-output words with f32 rounding.
"""
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            for row in range(4):
                ...
                residuals.append(f32(token0[row] + attention_output))
...
def main():
    parser = argparse.ArgumentParser(
        description="Print external token0 post-attention residual words.")
    ...
    for index, value in enumerate(result["embedding_words"]):
        print(f"oracle_token0_embedding_activation{index}_f32_hex: "
              f"0x{f32_bits(value):08x}")
    for index, value in enumerate(result["attention_outputs"]):

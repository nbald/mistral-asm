#!/usr/bin/env python3
"""External oracle for the token-0 layer-5 FFN down projection smoke.

This file is verification tooling, not runtime source. It reuses the external
layer-5 FFN RMSNorm oracle path, computes the full layer-5 FFN gate/up/SwiGLU
...
from token0_layer5_ffn_gate_oracle import LAYER5_FFN_GATE
...
from token0_layer5_ffn_up_oracle import LAYER5_FFN_UP


LAYER5_FFN_DOWN = "blk.5.ffn_down.weight"


def load_layer5_ffn_down_outputs(path, expected_epsilon_bits, activation,
    ...
    require(0 <= down_rows <= OUTPUT_WIDTH,
            "requested layer-5 FFN down row count outside output width")
    ...
    try:
        with mmap.mmap(fd, 0, access=mmap.ACCESS_READ) as buf:
            ...
            layer5_ffn_up = require_tensor(
                tensors, LAYER5_FFN_UP, GGML_TYPE_Q8_0, 2)
            layer5_ffn_down = require_tensor(
                tensors, LAYER5_FFN_DOWN, GGML_TYPE_Q8_0, 2)
            require(layer5_ffn_gate["dims"][0] == OUTPUT_WIDTH,
                    "layer-5 FFN gate input width mismatch")
            require(layer5_ffn_gate["dims"][1] == FFN_SWIGLU_WIDTH,
                    "layer-5 FFN gate output width mismatch")
            require(layer5_ffn_up["dims"][0] == OUTPUT_WIDTH,
                    "layer-5 FFN up input width mismatch")
            require(layer5_ffn_up["dims"][1] == FFN_SWIGLU_WIDTH,
                    "layer-5 FFN up output width mismatch")
            require(layer5_ffn_down["dims"][0] == FFN_SWIGLU_WIDTH,
                    "layer-5 FFN down input width mismatch")
            require(layer5_ffn_down["dims"][1] == OUTPUT_WIDTH,
                    "layer-5 FFN down output width mismatch")
            ...
            up_row_bytes = q8_0_row_bytes(layer5_ffn_up["dims"][0])
            up_start = tensor_pointer(
                buf, tensor_data_offset, layer5_ffn_up,
                up_row_bytes * layer5_ffn_up["dims"][1])
            down_row_bytes = q8_0_row_bytes(layer5_ffn_down["dims"][0])
            down_start = tensor_pointer(
                buf, tensor_data_offset, layer5_ffn_down,
                down_row_bytes * layer5_ffn_down["dims"][1])
            ...
            return {
                ...
                LAYER5_FFN_UP: layer5_ffn_up,
                LAYER5_FFN_DOWN: layer5_ffn_down,
                "layer5_ffn_gate_outputs": gate_outputs.copy(),
                "layer5_ffn_up_outputs": up_outputs.copy(),
                "layer5_ffn_swiglu_outputs": swiglu_outputs.copy(),
                "layer5_ffn_down_outputs": down_outputs.copy(),
            }
    finally:
        os.close(fd)
...
def run_oracle(path, down_rows=PUBLIC_OUTPUT_WORDS):
    ...
    down = load_layer5_ffn_down_outputs(
        path,
        result["epsilon_bits"],

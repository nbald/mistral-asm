...
Add a focused token-0 layer-3 FFN RMSNorm smoke that requires
`token0_layer3_post_attn_residual: 1`, applies `blk.3.ffn_norm.weight`,
publishes a status, and emits the first four guarded exact-hex activation
words.
...
- Layer-3 attention coverage now includes retained descriptor lookup, RMSNorm,
  Q/K/V matvecs, single-token grouped-query context expansion, guarded
  output-projection matvec words for `blk.3.attn_output.weight`, and a guarded
  post-attention residual handoff.
...
- Added `src/infer/token0_layer3_post_attn_residual.s`. It requires
  `token0_layer2_post_ffn_residual: 1`,
  `token0_layer3_attn_output_matvec: 1`, and the retained layer-3 attention
  output descriptor width `3072`, then fills private 3072-f32 residual storage.
- Published the first guarded layer-3 post-attention residual words:
...
- Added the focused external oracle script and note for the layer-3
  post-attention residual; the runtime/oracle diff is empty for the layer-2
  post-FFN residual, layer-3 attention output, and new layer-3 post-attention
  residual public labels.
...
- Review gate pass 2 for the completed layer-3 attention chain found no
  ...
  complete; feature work has resumed through the layer-3 post-attention
  residual.

## Known Blockers

- No functional blocker to the layer-3 FFN RMSNorm step.
- Add layer-3 FFN work in focused layer-3 files; do not grow the existing
  near-limit layer-2 modules for layer-3 work.
- Keep layer-3 output-projection work in `src/infer/token0_layer3_attn_output.s`
  and layer-3 post-attention residual work in
...
- `work/oracle/token0_layer3_post_attn_residual_oracle.py`
- `work/oracle/token0-layer3-post-attn-residual.md`
...
Layer-3 post-attention residual verification passed:

- `make clean all check`
- post-help-update `make all check`
- `./mistral-asm --help`
- real-target run reported `token0_layer3_attn_output_matvec: 1` and
  `token0_layer3_post_attn_residual: 1`
- focused runtime/oracle diff was empty for the layer-2 post-FFN residual,
  layer-3 attention output, and layer-3 post-attention residual public labels
- 24-byte header-only GGUF kept `layer3_attn_output_tensor_found: 0`,
  `token0_layer3_attn_output_matvec: 0`, and
  `token0_layer3_post_attn_residual: 0`, and emitted no guarded layer-3 output
  or post-attention residual exact-hex labels
...
- layer-3 post-attention residual symbol inspection

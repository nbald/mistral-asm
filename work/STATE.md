...
Add a guarded token-0 value-projection matvec smoke through the retained
`blk.0.attn_v.weight` descriptor, printing only a status line and no value
output slice yet.
...
- Scalar Q8_0 helpers, token-0 embedding dequantization, scalar f32 RMSNorm,
  query projection, and key projection smokes are wired. Query/key output slices
  have external oracle notes and exact-hex comparisons.
- The value projection step completed in this state is descriptor-only:
  `_start` prints `attn_v_tensor_*` summary fields, but no
  `blk.0.attn_v.weight` payload bytes are read yet.
...
- `./mistral-asm --help` returned status 0 with the value-descriptor milestone
...
- The real target model under `models/` returned status 0, printed
  ...
  offset 457924608, and kept existing token-0 embedding, RMSNorm, query, and
  key smoke statuses at 1 with the previous query/key exact-hex output words.
- `strace -e trace=mmap,munmap,close` on the real target returned status 0,
  showed `close(3) = 0`, successful value descriptor output, and
  `munmap(..., 3651679520) = 0`.

...
## 2026-05-11T21:53:31+02:00

- Added guarded status-only layer-4 attention query matvec coverage in the
  focused layer-4 module. The smoke requires the retained layer-4 attention
  RMSNorm activation, exact Q8_0 `[3072,4096]` `blk.4.attn_q.weight`, and a
  bounded payload span before filling private query output storage.
- Verification evidence: the real target reported `token0_layer4_attn_q_matvec:
  1` while keeping the existing layer-3 post-FFN residual and layer-4 RMSNorm
...
## 2026-05-11T22:16:09+02:00

- Added guarded status-only layer-4 attention key matvec coverage in the
  focused layer-4 module. The smoke requires the retained layer-4 attention
  RMSNorm activation, exact Q8_0 `[3072,1024]` `blk.4.attn_k.weight`, and a
  bounded payload span before filling private key output storage.
- Verification evidence: the real target reported `token0_layer4_attn_k_matvec:
...
## 2026-05-12T00:17:49+02:00

- Added descriptor-only layer-4 FFN gate setup. The new retained slot records
  `blk.4.ffn_gate.weight` directory metadata only; no layer-4 FFN gate matvec,
...
- Verification evidence: the real target reports the descriptor as Q8_0
  `[3072 x 9216]` at relative offset `986112000`, while the already-published
  layer-4 attention output, post-attention residual, and FFN RMSNorm slices
...
## 2026-05-12T00:28:26+02:00

- Published the first guarded layer-4 FFN gate matvec exact-hex slice. The first
  four words are `0x3ee0150a`, `0xbdd9edb2`, `0xbfbf1ff1`, and `0x3f5b31a5`.
- Added a focused external oracle that reuses the full layer-4 FFN RMSNorm
  activation and dots it with the first four rows of `blk.4.ffn_gate.weight`
...
## 2026-05-12T02:27:47+02:00

- Added status-only layer-5 attention query matvec coverage. The smoke consumes
  the private layer-5 RMSNorm activation only after its status is `1`, proves
  the retained `blk.5.attn_q.weight` Q8_0 `[3072 x 4096]` payload span, writes
  a private 4096-f32 query output buffer, and intentionally publishes no query
...
## 2026-05-12T02:40:44+02:00

- Added descriptor-only layer-5 attention key coverage. The runtime now retains
  and prints `blk.5.attn_k.weight` as Q8_0 `[3072 x 1024]` at relative offset
  `1046286336` on the local target GGUF.
- Verification evidence: clean rebuild/check passed, the existing layer-5
...
## 2026-05-12T03:14:04+02:00

- Published the first guarded layer-5 attention value exact-hex slice from the
  private value projection buffer. The first four words are `0x3c308045`,
  `0x3af0e7ba`, `0xbc4405eb`, and `0xbb67db55`.
- Added a focused external oracle that reuses the full layer-5 key oracle path,
  then dots the same RMSNorm activation against the first four rows of
  `blk.5.attn_v.weight`. This preserves existing query/key coverage while
  adding value rows.
- Verification evidence: the real-target runtime/oracle comparison matched the
  52 public exact-hex labels covered by the new oracle, plus the epsilon
...
## 2026-05-12T05:24:06+02:00

- Review gate pass 2 over the completed layer-5 attention Q/K/V handoff,
  single-token context, output-projection matvec/slice, and post-attention
  residual handoff chain found no blocking runtime issues.
- Verification evidence: clean build/check and oracle compile passed; the
  real-target comparison matched all 61 covered labels; the packed zero-count

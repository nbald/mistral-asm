...
# Contract: run the token-0 layer-4 FFN up matvec smoke and publish only its
# status line.
# Inputs: no register inputs. Reads the live mapping handoff slots, the retained
...
# private token0_layer4_ffn_up_output buffer with 9216 f32 values for later
# focused layer-4 FFN work. Always prints exactly one status label/value/newline
# sequence to stdout. The return register is unspecified.
# Clobbers: caller-saved registers, xmm0, xmm1, xmm2 and flags. The matvec
# helper preserves any callee-saved registers it uses internally.
# Ownership/lifetime: borrows the model mmap and the layer-4 FFN-normalized
...
# Error behavior: status is 1 only after a bounded Q8_0 matvec completes;
# otherwise status is 0 and no layer-4 FFN up payload bytes are read. This
# status-only step intentionally publishes no exact-hex output slice. Output

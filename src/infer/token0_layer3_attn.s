...
# Contract: run the token-0 layer-3 attention RMSNorm smoke and publish only
# its status line.
...
# label/value/newline sequence to stdout; no exact-hex activation slice is
# printed in this status-only step. The return register is unspecified.
...
# status is 0 and no layer-3 attention norm payload bytes are read. Output write

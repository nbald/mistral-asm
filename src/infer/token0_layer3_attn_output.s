...
token0_layer3_attn_output0_f32_text:
	.ascii "token0_layer3_attn_output0_f32_hex: "
token0_layer3_attn_output0_f32_text_end:

token0_layer3_attn_output1_f32_text:
	.ascii "token0_layer3_attn_output1_f32_hex: "
token0_layer3_attn_output1_f32_text_end:

token0_layer3_attn_output2_f32_text:
	.ascii "token0_layer3_attn_output2_f32_hex: "
token0_layer3_attn_output2_f32_text_end:

token0_layer3_attn_output3_f32_text:
	.ascii "token0_layer3_attn_output3_f32_hex: "
token0_layer3_attn_output3_f32_text_end:
...
# otherwise status is 0, no layer-3 output-projection matrix payload bytes are
...
run_token0_layer3_attn_output_matvec_status:
	...
	call print_token0_layer3_attn_output_slice
	ret
...
.type print_token0_layer3_attn_output_slice, @function
...
# Inputs: no register inputs. Reads token0_layer3_attn_output_matvec_status and
# the first four f32 words of token0_layer3_attn_output.
# Outputs: writes four labeled raw f32 bit patterns to stdout when
# token0_layer3_attn_output_matvec_status is 1; writes nothing otherwise.
# Clobbers: caller-saved registers and flags through sys_write and
# write_u32_hex.
# Ownership/lifetime: reads private module-owned layer-3 output-projection
...
print_token0_layer3_attn_output_slice:
	cmp qword ptr [rip + token0_layer3_attn_output_matvec_status], 1
	jne .Lprint_layer3_attn_output_slice_done

	mov rdi, 1
	lea rsi, [rip + token0_layer3_attn_output0_f32_text]
	mov rdx, token0_layer3_attn_output0_f32_text_end - token0_layer3_attn_output0_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer3_attn_output]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer3_attn_output1_f32_text]
	mov rdx, token0_layer3_attn_output1_f32_text_end - token0_layer3_attn_output1_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer3_attn_output + 4]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer3_attn_output2_f32_text]
	mov rdx, token0_layer3_attn_output2_f32_text_end - token0_layer3_attn_output2_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer3_attn_output + 8]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer3_attn_output3_f32_text]
	mov rdx, token0_layer3_attn_output3_f32_text_end - token0_layer3_attn_output3_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer3_attn_output + 12]
	call write_u32_hex
...
.Lprint_layer3_attn_output_slice_done:
	ret

.size print_token0_layer3_attn_output_slice, . - print_token0_layer3_attn_output_slice

...
.equ TOKEN0_LAYER2_ATTN_OUTPUT_VALUES, 3072
.equ TOKEN0_LAYER2_ATTN_OUTPUT_BYTES, TOKEN0_LAYER2_ATTN_OUTPUT_VALUES * 4

.section .rodata

token0_layer2_attn_output_matvec_text:
...
.global token0_layer2_attn_output_matvec_status
.balign 8
token0_layer2_attn_output_matvec_status:
	.skip 8

.global token0_layer2_attn_output
.balign 4
token0_layer2_attn_output:
	.skip TOKEN0_LAYER2_ATTN_OUTPUT_BYTES

.section .text

.global run_token0_layer2_attn_output_matvec_status
.type run_token0_layer2_attn_output_matvec_status, @function

# Contract: run the token-0 layer-2 attention output-projection matvec smoke
# and publish its status line.
# Inputs: no register inputs. Reads the live mapping handoff slots, retained
# blk.2.attn_output.weight descriptor, token0_layer2_attn_context_status, and
...
# Ownership/lifetime: borrows the model mmap and layer-2 attention context only
...
run_token0_layer2_attn_output_matvec_status:
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer2_attn_output_matvec_text]
	mov rdx, token0_layer2_attn_output_matvec_text_end - token0_layer2_attn_output_matvec_text
	call sys_write

	mov rdi, 1
	mov rsi, qword ptr [rip + token0_layer2_attn_output_matvec_status]
...
.type token0_layer2_attn_output_matvec_smoke, @function

# Contract: opportunistically project the token-0 layer-2 single-token attention
# context through the reusable blk.2.attn_output.weight matrix.
# Inputs: no register inputs. Reads the process-owned layer2_attn_output tensor
# slot, live mapping descriptor, token0_layer2_attn_context_status, and
# token0_layer2_attn_context.
# Outputs: rax = 1 when the layer-2 context is available and
# blk.2.attn_output.weight is exactly a two-dimensional Q8_0 [4096 x 3072]
# matrix whose complete payload span fits inside the mapping, after
# q8_0_matvec_f32 writes token0_layer2_attn_output; otherwise rax = 0 and no
# layer-2 output-projection matrix payload bytes are read.
...
# q8_0_matvec_f32, reads the static layer-2 context as the shared f32 input
# vector, and writes exactly TOKEN0_LAYER2_ATTN_OUTPUT_BYTES into private static
...
# skip with status 0.
token0_layer2_attn_output_matvec_smoke:
	xor eax, eax
	cmp qword ptr [rip + token0_layer2_attn_context_status], 1
	jne .Llayer2_attn_output_smoke_done
	...
	jne .Llayer2_attn_output_smoke_done
	cmp qword ptr [rip + layer2_attn_output_tensor_dim1], TOKEN0_LAYER2_ATTN_OUTPUT_VALUES
	jne .Llayer2_attn_output_smoke_done

	# Tensor offsets are relative to the aligned tensor-data base. Resolve the
	# layer-2 output-projection matrix start and prove the complete row-major
	...
	test rax, rax
	js .Llayer2_attn_output_smoke_skip
	mov rdx, qword ptr [rip + layer2_attn_output_tensor_offset]
	test rdx, rdx
	js .Llayer2_attn_output_smoke_skip
	add rax, rdx
	jc .Llayer2_attn_output_smoke_skip

	mov r10, qword ptr [rip + gguf_mapping_size]
	cmp rax, r10
	jae .Llayer2_attn_output_smoke_skip

	mov r8, TOKEN0_LAYER2_ATTN_CONTEXT_VALUES
	mov r9, r8
	shr r9, 5
	mov r11, r9
	imul r11, r11, Q8_0_BLOCK_BYTES
	jo .Llayer2_attn_output_smoke_skip
	mov rcx, TOKEN0_LAYER2_ATTN_OUTPUT_VALUES
	mov rdx, rcx
	imul rdx, r11
	jo .Llayer2_attn_output_smoke_skip
	...
	sub r11, rax
	cmp r11, rdx
	jb .Llayer2_attn_output_smoke_skip

	mov rdi, qword ptr [rip + gguf_mapping_base]
	test rdi, rdi
	jz .Llayer2_attn_output_smoke_skip
	add rdi, rax
	jc .Llayer2_attn_output_smoke_skip

	lea rsi, [rip + token0_layer2_attn_context]
...
.Llayer2_attn_output_smoke_skip:
	xor eax, eax

.Llayer2_attn_output_smoke_done:
	ret

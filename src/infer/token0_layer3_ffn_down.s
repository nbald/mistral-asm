...
# Contract: run the token-0 layer-3 FFN down matvec smoke and publish only its
# status line.
...
# output words are printed in this status-only step. The return register is
# unspecified.
...
# output bytes are published. Output write errors are diagnostic-only and are

...
.equ TOKEN0_LAYER3_POST_FFN_RESIDUAL_VALUES, 3072
.equ TOKEN0_LAYER3_POST_FFN_RESIDUAL_BYTES, TOKEN0_LAYER3_POST_FFN_RESIDUAL_VALUES * 4
...
token0_layer3_post_ffn_residual_text:
	.ascii "token0_layer3_post_ffn_residual: "
token0_layer3_post_ffn_residual_text_end:

token0_layer3_post_ffn_residual0_f32_text:
	.ascii "token0_layer3_post_ffn_residual0_f32_hex: "
token0_layer3_post_ffn_residual0_f32_text_end:

token0_layer3_post_ffn_residual1_f32_text:
	.ascii "token0_layer3_post_ffn_residual1_f32_hex: "
token0_layer3_post_ffn_residual1_f32_text_end:

token0_layer3_post_ffn_residual2_f32_text:
	.ascii "token0_layer3_post_ffn_residual2_f32_hex: "
token0_layer3_post_ffn_residual2_f32_text_end:

token0_layer3_post_ffn_residual3_f32_text:
	.ascii "token0_layer3_post_ffn_residual3_f32_hex: "
token0_layer3_post_ffn_residual3_f32_text_end:
...
.global token0_layer3_post_ffn_residual_status
.balign 8
token0_layer3_post_ffn_residual_status:
	.skip 8

.global token0_layer3_post_ffn_residual
.balign 4
token0_layer3_post_ffn_residual:
	.skip TOKEN0_LAYER3_POST_FFN_RESIDUAL_BYTES
...
.global run_token0_layer3_post_ffn_residual_status
.type run_token0_layer3_post_ffn_residual_status, @function

# Contract: run the token-0 layer-3 post-FFN residual smoke and publish its
# status line plus the fixed exact-hex oracle slice on success.
# Inputs: no register inputs. Reads token0_layer3_post_attn_residual_status,
# token0_layer3_ffn_down_matvec_status, layer3_ffn_down_tensor_dim1,
# token0_layer3_post_attn_residual, and this module's retained layer-3 FFN down
# output.
# Outputs: writes token0_layer3_post_ffn_residual_status and, on success, fills
# token0_layer3_post_ffn_residual with 3072 scalar f32 residual sums. Always
# prints exactly one status label/value/newline sequence to stdout and prints
...
# Ownership/lifetime: borrows the retained layer-3 post-attention residual and
# this module's private FFN-down output only during this call; owns the layer-3
# post-FFN residual status and retained output storage for later focused
# inference steps.
# Error behavior: status is 1 only after both prerequisite statuses are present
# and the retained layer-3 FFN-down descriptor still proves a 3072-wide output.
# Otherwise status is 0 and no post-FFN residual bytes are written or printed.
# Output write errors remain diagnostic-only.
run_token0_layer3_post_ffn_residual_status:
	call token0_layer3_post_ffn_residual_smoke
	mov qword ptr [rip + token0_layer3_post_ffn_residual_status], rax

	mov rdi, 1
	lea rsi, [rip + token0_layer3_post_ffn_residual_text]
	mov rdx, token0_layer3_post_ffn_residual_text_end - token0_layer3_post_ffn_residual_text
	call sys_write

	mov rdi, 1
	mov rsi, qword ptr [rip + token0_layer3_post_ffn_residual_status]
	call write_u64_decimal
	...
	call print_token0_layer3_post_ffn_residual_slice
	ret

.size run_token0_layer3_post_ffn_residual_status, . - run_token0_layer3_post_ffn_residual_status

.type print_token0_layer3_post_ffn_residual_slice, @function

# Contract: print a fixed exact-hex slice from the token-0 layer-3 post-FFN
# residual when that smoke path succeeded.
# Inputs: no register inputs. Reads token0_layer3_post_ffn_residual_status and
# the first four f32 words of token0_layer3_post_ffn_residual.
# Outputs: writes four labeled raw f32 bit patterns to stdout when
# token0_layer3_post_ffn_residual_status is 1; writes nothing otherwise.
# Clobbers: caller-saved registers and flags through sys_write and
# write_u32_hex.
# Ownership/lifetime: reads private module-owned layer-3 post-FFN residual
...
print_token0_layer3_post_ffn_residual_slice:
	cmp qword ptr [rip + token0_layer3_post_ffn_residual_status], 1
	jne .Lprint_layer3_post_ffn_residual_slice_done

	mov rdi, 1
	lea rsi, [rip + token0_layer3_post_ffn_residual0_f32_text]
	mov rdx, token0_layer3_post_ffn_residual0_f32_text_end - token0_layer3_post_ffn_residual0_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer3_post_ffn_residual]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer3_post_ffn_residual1_f32_text]
	mov rdx, token0_layer3_post_ffn_residual1_f32_text_end - token0_layer3_post_ffn_residual1_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer3_post_ffn_residual + 4]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer3_post_ffn_residual2_f32_text]
	mov rdx, token0_layer3_post_ffn_residual2_f32_text_end - token0_layer3_post_ffn_residual2_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer3_post_ffn_residual + 8]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer3_post_ffn_residual3_f32_text]
	mov rdx, token0_layer3_post_ffn_residual3_f32_text_end - token0_layer3_post_ffn_residual3_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer3_post_ffn_residual + 12]
	call write_u32_hex
...
.Lprint_layer3_post_ffn_residual_slice_done:
	ret

.size print_token0_layer3_post_ffn_residual_slice, . - print_token0_layer3_post_ffn_residual_slice
...
.type token0_layer3_post_ffn_residual_smoke, @function

# Contract: derive the token-0 layer-3 post-FFN residual activation.
# Inputs: no register inputs. Reads token0_layer3_post_attn_residual_status,
# token0_layer3_ffn_down_matvec_status, layer3_ffn_down_tensor_dim1,
# token0_layer3_post_attn_residual, and token0_layer3_ffn_down_output.
# Outputs: rax = 1 after writing 3072 f32 sums to
# token0_layer3_post_ffn_residual; otherwise rax = 0 and no layer-3 post-FFN
...
token0_layer3_post_ffn_residual_smoke:
	xor eax, eax
	cmp qword ptr [rip + token0_layer3_post_attn_residual_status], 1
	jne .Llayer3_post_ffn_residual_done
	cmp qword ptr [rip + token0_layer3_ffn_down_matvec_status], 1
	jne .Llayer3_post_ffn_residual_done
	...
	cmp qword ptr [rip + layer3_ffn_down_tensor_dim1], TOKEN0_LAYER3_POST_FFN_RESIDUAL_VALUES
	jne .Llayer3_post_ffn_residual_done

	lea rsi, [rip + token0_layer3_post_attn_residual]
	lea rdx, [rip + token0_layer3_ffn_down_output]
	lea rdi, [rip + token0_layer3_post_ffn_residual]
	mov rcx, TOKEN0_LAYER3_POST_FFN_RESIDUAL_VALUES

.Llayer3_post_ffn_residual_loop:
	...
	add rdi, 4
	dec rcx
	jnz .Llayer3_post_ffn_residual_loop

	mov eax, 1

.Llayer3_post_ffn_residual_done:
	ret

.size token0_layer3_post_ffn_residual_smoke, . - token0_layer3_post_ffn_residual_smoke

...
.equ TOKEN0_LAYER2_FFN_SWIGLU_BYTES, TOKEN0_LAYER2_FFN_SWIGLU_VALUES * 4
...
token0_layer2_ffn_swiglu_text:
	.ascii "token0_layer2_ffn_swiglu: "
token0_layer2_ffn_swiglu_text_end:

token0_layer2_ffn_swiglu_output0_f32_text:
	.ascii "token0_layer2_ffn_swiglu_output0_f32_hex: "
token0_layer2_ffn_swiglu_output0_f32_text_end:

token0_layer2_ffn_swiglu_output1_f32_text:
	.ascii "token0_layer2_ffn_swiglu_output1_f32_hex: "
token0_layer2_ffn_swiglu_output1_f32_text_end:

token0_layer2_ffn_swiglu_output2_f32_text:
	.ascii "token0_layer2_ffn_swiglu_output2_f32_hex: "
token0_layer2_ffn_swiglu_output2_f32_text_end:

token0_layer2_ffn_swiglu_output3_f32_text:
	.ascii "token0_layer2_ffn_swiglu_output3_f32_hex: "
token0_layer2_ffn_swiglu_output3_f32_text_end:
...
.global token0_layer2_ffn_swiglu_status
.balign 8
...
.global run_token0_layer2_ffn_swiglu_status
.type run_token0_layer2_ffn_swiglu_status, @function

# Contract: run the token-0 layer-2 FFN SwiGLU smoke and publish its status
# line plus the fixed exact-hex oracle slice on success.
# Inputs: no register inputs. Reads token0_layer2_ffn_gate_matvec_status,
# token0_layer2_ffn_up_matvec_status, token0_layer2_ffn_gate_output, and
# token0_layer2_ffn_up_output.
# Outputs: writes token0_layer2_ffn_swiglu_status and, on success, fills the
# private token0_layer2_ffn_swiglu_output buffer. Always prints exactly one
# status label/value/newline sequence to stdout and prints the first four
# exact-hex output words only when the status is 1. The return register is
# unspecified.
...
run_token0_layer2_ffn_swiglu_status:
	call token0_layer2_ffn_swiglu_smoke
	mov qword ptr [rip + token0_layer2_ffn_swiglu_status], rax

	mov rdi, 1
	lea rsi, [rip + token0_layer2_ffn_swiglu_text]
	mov rdx, token0_layer2_ffn_swiglu_text_end - token0_layer2_ffn_swiglu_text
	call sys_write

	mov rdi, 1
	mov rsi, qword ptr [rip + token0_layer2_ffn_swiglu_status]
	call write_u64_decimal
	...
	call print_token0_layer2_ffn_swiglu_output_slice
	ret

.size run_token0_layer2_ffn_swiglu_status, . - run_token0_layer2_ffn_swiglu_status

.type print_token0_layer2_ffn_swiglu_output_slice, @function

# Contract: print a fixed exact-hex slice from the token-0 layer-2 FFN SwiGLU
# activation when that smoke path succeeded.
# Inputs: no register inputs. Reads token0_layer2_ffn_swiglu_status and the
# first four f32 words of token0_layer2_ffn_swiglu_output.
# Outputs: writes four labeled raw f32 bit patterns to stdout when
# token0_layer2_ffn_swiglu_status is 1; writes nothing otherwise.
# Clobbers: caller-saved registers and flags through sys_write and
# write_u32_hex.
# Ownership/lifetime: reads private module-owned layer-2 FFN SwiGLU activation
# storage only during this call and does not retain pointers.
# Error behavior: this is summary output for oracle comparison; write failures
# are intentionally not surfaced separately.
print_token0_layer2_ffn_swiglu_output_slice:
	cmp qword ptr [rip + token0_layer2_ffn_swiglu_status], 1
	jne .Lprint_layer2_ffn_swiglu_output_slice_done

	mov rdi, 1
	lea rsi, [rip + token0_layer2_ffn_swiglu_output0_f32_text]
	mov rdx, token0_layer2_ffn_swiglu_output0_f32_text_end - token0_layer2_ffn_swiglu_output0_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer2_ffn_swiglu_output]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer2_ffn_swiglu_output1_f32_text]
	mov rdx, token0_layer2_ffn_swiglu_output1_f32_text_end - token0_layer2_ffn_swiglu_output1_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer2_ffn_swiglu_output + 4]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer2_ffn_swiglu_output2_f32_text]
	mov rdx, token0_layer2_ffn_swiglu_output2_f32_text_end - token0_layer2_ffn_swiglu_output2_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer2_ffn_swiglu_output + 8]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer2_ffn_swiglu_output3_f32_text]
	mov rdx, token0_layer2_ffn_swiglu_output3_f32_text_end - token0_layer2_ffn_swiglu_output3_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer2_ffn_swiglu_output + 12]
	call write_u32_hex
...
.Lprint_layer2_ffn_swiglu_output_slice_done:
	ret

.size print_token0_layer2_ffn_swiglu_output_slice, . - print_token0_layer2_ffn_swiglu_output_slice

.type token0_layer2_ffn_swiglu_smoke, @function

# Contract: derive the token-0 layer-2 FFN SwiGLU activation from retained gate
# and up projection outputs.
# Inputs: no register inputs. Reads token0_layer2_ffn_gate_matvec_status,
# token0_layer2_ffn_up_matvec_status, token0_layer2_ffn_gate_output, and
# token0_layer2_ffn_up_output.
# Outputs: rax = 1 after writing TOKEN0_LAYER2_FFN_SWIGLU_VALUES f32 values to
# token0_layer2_ffn_swiglu_output, each equal to
# silu(token0_layer2_ffn_gate_output[i]) * token0_layer2_ffn_up_output[i];
...
# Error behavior: this is a smoke gate for the layer-2 FFN activation, not final
# graph setup. Missing prerequisite projection statuses are skipped with status
# 0.
token0_layer2_ffn_swiglu_smoke:
	xor eax, eax
	cmp qword ptr [rip + token0_layer2_ffn_gate_matvec_status], 1
	jne .Llayer2_ffn_swiglu_done
	cmp qword ptr [rip + token0_layer2_ffn_up_matvec_status], 1
	jne .Llayer2_ffn_swiglu_done

	# Projection statuses prove both retained buffers have the target 9216-f32
	# width, so the activation step only combines those private rows.
	lea rdi, [rip + token0_layer2_ffn_gate_output]
	lea rsi, [rip + token0_layer2_ffn_up_output]
	lea rdx, [rip + token0_layer2_ffn_swiglu_output]
	mov ecx, TOKEN0_LAYER2_FFN_SWIGLU_VALUES
	call swiglu_f32

	mov eax, 1

.Llayer2_ffn_swiglu_done:
	ret

.size token0_layer2_ffn_swiglu_smoke, . - token0_layer2_ffn_swiglu_smoke

...
# Contract: run the token-0 layer-4 FFN down matvec smoke and publish only its
# status line.
...
# prints exactly one status label/value/newline sequence to stdout. No exact-hex
# down output labels are published in this status-only smoke step. The return
...
# otherwise status is 0, no layer-4 FFN down payload bytes are read, and no down
# output words are printed. Output write errors are diagnostic-only.

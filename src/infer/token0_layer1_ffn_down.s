...
.equ TOKEN0_LAYER1_POST_FFN_RESIDUAL_VALUES, 3072
.equ TOKEN0_LAYER1_POST_FFN_RESIDUAL_BYTES, TOKEN0_LAYER1_POST_FFN_RESIDUAL_VALUES * 4
...
token0_layer1_ffn_down_output0_f32_text:
	.ascii "token0_layer1_ffn_down_output0_f32_hex: "
token0_layer1_ffn_down_output0_f32_text_end:

token0_layer1_ffn_down_output1_f32_text:
	.ascii "token0_layer1_ffn_down_output1_f32_hex: "
token0_layer1_ffn_down_output1_f32_text_end:

token0_layer1_ffn_down_output2_f32_text:
	.ascii "token0_layer1_ffn_down_output2_f32_hex: "
token0_layer1_ffn_down_output2_f32_text_end:

token0_layer1_ffn_down_output3_f32_text:
	.ascii "token0_layer1_ffn_down_output3_f32_hex: "
token0_layer1_ffn_down_output3_f32_text_end:

token0_layer1_post_ffn_residual_text:
...
.global token0_layer1_post_ffn_residual_status
.balign 8
token0_layer1_post_ffn_residual_status:
	.skip 8

.global token0_layer1_post_ffn_residual
.balign 4
token0_layer1_post_ffn_residual:
	.skip TOKEN0_LAYER1_POST_FFN_RESIDUAL_BYTES
...
run_token0_layer1_ffn_down_matvec_status:
	...
	call print_token0_layer1_ffn_down_output_slice
	ret
...
# by entry state and this module's retained FFN down output only during this
# call; owns the layer-1 post-FFN residual status and output storage for later
...
run_token0_layer1_post_ffn_residual_status:
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer1_post_ffn_residual_text]
	mov rdx, token0_layer1_post_ffn_residual_text_end - token0_layer1_post_ffn_residual_text
	call sys_write

	mov rdi, 1
	mov rsi, qword ptr [rip + token0_layer1_post_ffn_residual_status]
	call write_u64_decimal
...
.type print_token0_layer1_ffn_down_output_slice, @function

# Contract: print a fixed exact-hex slice from the token-0 layer-1 FFN down
# projection output when that smoke path succeeded.
# Inputs: no register inputs. Reads token0_layer1_ffn_down_matvec_status and the
# first four f32 words of token0_layer1_ffn_down_output.
# Outputs: writes four labeled raw f32 bit patterns to stdout when
# token0_layer1_ffn_down_matvec_status is 1; writes nothing otherwise.
# Clobbers: caller-saved registers and flags through sys_write and
# write_u32_hex.
# Ownership/lifetime: reads private module-owned layer-1 FFN down output storage
...
print_token0_layer1_ffn_down_output_slice:
	cmp qword ptr [rip + token0_layer1_ffn_down_matvec_status], 1
	jne .Lprint_layer1_ffn_down_output_slice_done

	mov rdi, 1
	lea rsi, [rip + token0_layer1_ffn_down_output0_f32_text]
	mov rdx, token0_layer1_ffn_down_output0_f32_text_end - token0_layer1_ffn_down_output0_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer1_ffn_down_output]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer1_ffn_down_output1_f32_text]
	mov rdx, token0_layer1_ffn_down_output1_f32_text_end - token0_layer1_ffn_down_output1_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer1_ffn_down_output + 4]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer1_ffn_down_output2_f32_text]
	mov rdx, token0_layer1_ffn_down_output2_f32_text_end - token0_layer1_ffn_down_output2_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer1_ffn_down_output + 8]
	call write_u32_hex
	...
	mov rdi, 1
	lea rsi, [rip + token0_layer1_ffn_down_output3_f32_text]
	mov rdx, token0_layer1_ffn_down_output3_f32_text_end - token0_layer1_ffn_down_output3_f32_text
	call sys_write

	mov rdi, 1
	mov esi, dword ptr [rip + token0_layer1_ffn_down_output + 12]
	call write_u32_hex
...
.Lprint_layer1_ffn_down_output_slice_done:
	ret
...
# Contract: derive the token-0 layer-1 post-FFN residual activation.
# Inputs: no register inputs. Reads token0_layer1_post_attn_residual_status,
# token0_layer1_ffn_down_matvec_status, token0_layer1_post_attn_residual, and
# token0_layer1_ffn_down_output.
# Outputs: rax = 1 after writing 3072 f32 sums to
# token0_layer1_post_ffn_residual; otherwise rax = 0 and no layer-1 post-FFN
...
# not final layer execution. Missing prerequisite statuses are skipped with
# status 0.
token0_layer1_post_ffn_residual_smoke:
	xor eax, eax
	cmp qword ptr [rip + token0_layer1_post_attn_residual_status], 1
	jne .Llayer1_post_ffn_residual_done
	cmp qword ptr [rip + token0_layer1_ffn_down_matvec_status], 1
	jne .Llayer1_post_ffn_residual_done

	# The two prerequisite statuses prove both 3072-wide static rows were
	# freshly written for this token path, so this step only adds retained f32
	# buffers and publishes a reusable residual for the next layer.
	lea rsi, [rip + token0_layer1_post_attn_residual]
	lea rdx, [rip + token0_layer1_ffn_down_output]
	lea rdi, [rip + token0_layer1_post_ffn_residual]
	mov rcx, TOKEN0_LAYER1_POST_FFN_RESIDUAL_VALUES

.Llayer1_post_ffn_residual_loop:
	...
	add rdi, 4
	dec rcx
	jnz .Llayer1_post_ffn_residual_loop

	mov eax, 1

.Llayer1_post_ffn_residual_done:
	ret

.size token0_layer1_post_ffn_residual_smoke, . - token0_layer1_post_ffn_residual_smoke
